...
Ensure curl_device_manager.sh is available:
...
📊 Chart Types
Daily Empty Session Percentage - Tracks empty session trends
Daily Microsession Percentage - Shows low-energy session patterns
***🛡️ Error Handling
...
pip install matplotlib
🔍 Getting Help
Enable debug mode with --debug for detailed logging
Check API connectivity by running without analysis flags
Validate date ranges - ensure they contain session data
Verify authentication - ensure curl_device_manager.sh works independently
...
***🎁 Pro Tips
💡 Performance Tips
Use smaller date ranges for faster analysis of large datasets
Increase limit parameter to reduce API calls for comprehensive analysis
Run without --graph first to check data quality before generating charts
Use --debug selectively - it produces verbose output
🔧 Automation Tips
Always use --printsessions in scripts to avoid interactive prompts
Pipe output to files for further processing: seshis.py --empty > report.txt
Combine with cron jobs for regular monitoring
Use specific date ranges in automation rather than "today" for consistency
📊 Analysis Tips
Start with --empty --micro to get overview before diving deep
Use custom date ranges to analyze specific incidents or time periods
Compare different thresholds for microsession analysis to find optimal values
Look for patterns in daily breakdowns to identify systematic issues
***🤝 Contributing
...
Run with --debug to gather diagnostic information
